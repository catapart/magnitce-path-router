...
export class PathRouterElement extends HTMLElement
{
    ...
    #getRouteElements(path: string)
    {
        ...
        let foundPage: RoutePageElement|undefined = pagePath == ""
        ? this.defaultRoute
        : this.#getRouteElement<RoutePageElement>(pagePathArray, this.#routeMap_pathToPage);
    ...
    #getRouteElement<T extends RoutePageElement|RouteDialogElement>(pagePathArray: string[], routeMap: Map<string, RoutePageElement|RouteDialogElement>)
    {
        ...
        for(let [routePath, element] of routeMap)
        {
            ...
            for(let i = pagePathArray.length; i < routeArray.length; i++)
            {
                ...
                if(slug != WILDCARD_CHARACTER && slug.startsWith(':') == false)
                {
    ...
    #pathArraySelectsRouteArray(pathArray: string[], routeArray: string[])
    {
        ...
        for(let i = 0; i < pathArray.length; i++)
        {
            ...
            if(slug == null)
            {
                if(lastRouteSlugPermitsMatch == true)
                {
                    continue;
                }
    ...
    getPathValuePairs()
    {
        ...
        if(path == null) { return pairs; }
        const pathArray = path.split("/");

        // if the first slug is a property value
        // for the parent route, remove the first slug
        const parentRoute = this.parentElement?.closest('route-page,[is="route-dialog"]');
        if(parentRoute != null)
        {
            const parentRoutePathAttribute = parentRoute.getAttribute('path');
            if(parentRoutePathAttribute != null && parentRoutePathAttribute.indexOf(':') > -1)
            {
                pathArray.shift();
            }
        }
        ...
        for(let i = 0; i < pathArray.length; i++)
        {
            const valueItem = pathArray[i + 1];
            if(valueItem == null) { break; }
            const nameItem = pathArray[i];
            pairs[nameItem] = valueItem;
        }
