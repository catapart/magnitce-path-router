...
export default defineConfig({
    ...
    build: {
        ...
        rollupOptions: {
            ...
            {
                if(source.includes('path-router.ts') || source.includes('path-router.js') || source.includes('path-router.min.js')) { return true; }
            },
            output: {
    ...
    plugins: [{
        ...
        transform(code, id) {
            const libraryMatch = code.match(/['"].*?path-router.*?["']/g);
            if(libraryMatch == null) { return null; }
            // console.log(libraryMatch);
            const result = code.replace(libraryMatch[0], '"../libs/path-router.min.js"');
            // console.log(result);
            return {
                code: result,
