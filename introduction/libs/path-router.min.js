...
const RouteType = (elementType = HTMLElement) => {
  return class extends elementType {
    ...
    applyEventListener(type, listener, options) {
      ...
      if ((type == RouteEvent.BeforeOpen.toString() || type == RouteEvent.AfterOpen.toString()) && isOpen == true) {
        this.dispatchEvent(new Event(type));
      } else if (type == RouteEvent.BeforeClose.toString() || type == RouteEvent.AfterClose.toString() && isOpen == false) {
        this.dispatchEvent(new Event(type));
      }
    }
    addBlockingEventListener(eventName, handler) {
      switch (eventName) {
        case RouteEvent.BeforeOpen:
          this.#blockingBeforeOpen.push(handler);
          break;
        case RouteEvent.AfterOpen:
          this.#blockingAfterOpen.push(handler);
          break;
        case RouteEvent.BeforeClose:
          this.#blockingBeforeClose.push(handler);
          break;
        case RouteEvent.AfterClose:
    ...
    applyBlockingEventListener(eventName, handler) {
      ...
      if ((eventName == RouteEvent.BeforeOpen.toString() || eventName == RouteEvent.AfterOpen.toString()) && isOpen == true) {
        this.dispatchEvent(new Event(eventName));
      } else if (eventName == RouteEvent.BeforeClose.toString() || eventName == RouteEvent.AfterClose.toString() && isOpen == false) {
        this.dispatchEvent(new Event(eventName));
      }
    }
  };
};
const COMPONENT_TAG_NAME$2 = "route-dialog";
class RouteDialogElement extends RouteType(HTMLDialogElement) {
}
if (customElements.get(COMPONENT_TAG_NAME$2) == null) {
  customElements.define(COMPONENT_TAG_NAME$2, RouteDialogElement, { extends: "dialog" });
}
const COMPONENT_TAG_NAME$1 = "route-page";
class RoutePageElement extends RouteType() {
}
if (customElements.get(COMPONENT_TAG_NAME$1) == null) {
  customElements.define(COMPONENT_TAG_NAME$1, RoutePageElement);
}
const PathRouterEvent = {
...
const ROUTEPROPERTY_DATA_ATTRIBUTE_KEYWORD = "property";
const COMPONENT_TAG_NAME = "path-router";
class PathRouterElement extends HTMLElement {
  get routePages() {
    return Array.from(this.querySelectorAll(`:scope > ${COMPONENT_TAG_NAME$1}, ${COMPONENT_TAG_NAME} :not(${COMPONENT_TAG_NAME}) ${COMPONENT_TAG_NAME$1}`), (route) => route);
  }
  get routeDialogs() {
    return Array.from(this.querySelectorAll(`:scope > [is="${COMPONENT_TAG_NAME$2}"]`), (routeDialog) => routeDialog);
  }
  get routes() {
    return Array.from(this.querySelectorAll(`:scope > ${COMPONENT_TAG_NAME$1},${COMPONENT_TAG_NAME} :not(${COMPONENT_TAG_NAME}) ${COMPONENT_TAG_NAME$1},:scope > [is="${COMPONENT_TAG_NAME$2}"]`), (route) => route);
  }
  ...
  async #update(path, previousPath) {
    ...
    if (pageHasChanged || currentDialogPath != dialogPath) {
      ...
      if (closed != false) {
        ...
        for (let i = 0; i < matchingPageRoutes.length; i++) {
          const routeData = matchingPageRoutes[i];
          if (routeData.route.closest(`[is="${COMPONENT_TAG_NAME$2}"][open]`) != null) {
            openPagePromise = this.#openRoutePage(routeData.route, dialogPath);
            this.#assignRouteProperties(routeData.route, routeData.properties);
          }
        }
  ...
  routeMatchesPath(route, queryPath, previousMatches, _isDialog = false) {
    ...
    const pathType = route.closest(`[is="${COMPONENT_TAG_NAME$2}"]`) == null ? "Page" : "Dialog";
    if (pathType == "Page") {
      return this.routeTypeMatches(route, pagePathArray, routePathArray, `${COMPONENT_TAG_NAME$1}`, previousMatches);
    } else if (dialogPath == null) {
      return [false, {}];
    }
    const dialogPathArray = dialogPath.split("/");
    return this.routeTypeMatches(route, dialogPathArray, routePathArray, `${COMPONENT_TAG_NAME$1},[is="${COMPONENT_TAG_NAME$2}"]`, previousMatches);
  }
